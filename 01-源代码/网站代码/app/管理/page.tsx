...
export default function AdminPage() {
  ...
  const handleParseTable = async () => {
    ...
    setSyncResult(null)
    try {
      const response = await fetch('/管理/api/parse-table', {
        method: 'POST',
  ...
  const handleSync = async () => {
    ...
    setSyncResult(null)
    try {
      const response = await fetch('/管理/api/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          selectedRows,
      ...
      if (result.success) {
        ...
        // 保存已选择的内容，用于创建周报时预填充
        const selectedRowsData = rows.filter((row) => row.selected)
        sessionStorage.setItem('lastSyncedItems', JSON.stringify(selectedRowsData))
      } else {
        setSyncResult({
          success: false,
          error: result.error,
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <button
            onClick={() => router.push('/管理/周报编辑')}
            className="inline-flex items-center px-6 py-3 bg-primary-pink text-white rounded-lg font-medium hover:bg-primary-pink/90 transition-colors"
          >
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      className="mt-1 mr-4 h-5 w-5 text-primary-blue focus:ring-primary-blue border-neutral-300 rounded"
                    />
