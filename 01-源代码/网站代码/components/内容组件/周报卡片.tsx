...
/** 周报展示用类型（与 Contentlayer Newsletter 兼容，不依赖生成文件） */
export interface NewsletterDisplay {
  ...
  tags?: string[]
  published?: boolean
  editorialContent?: { code?: string }
  includedItems?: string | unknown[]
}
  ...
  return (
    <article className={`group ${className}`}>
      <Link href={`/newsletter/${newsletter.slug}`} className="block">
        <div className="bg-white rounded-card shadow-card border border-neutral-200 overflow-hidden hover:shadow-card-hover transition-shadow">
          {/* 封面图或占位 */}
          {newsletter.coverImage ? (
            ...
            <div className="w-full h-48 md:h-64 bg-neutral-100 flex items-center justify-center" aria-hidden>
